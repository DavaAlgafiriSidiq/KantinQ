                ...
                <div class="col-md-3 mb-4">
                    <div class="card h-100 border-0 shadow-sm" style="border-radius: 15px; overflow: hidden;">
                        ...
                        <div class="card-body p-3 d-flex flex-column">
                            <span class="badge bg-light text-dark mb-2 align-self-start" style="font-size: 0.75rem; border: 1px solid #ddd;">
                            ...
                            <p class="mb-2 small fw-bold">
                                @if(!$habis)
                                    <span class="text-success"><i class="fas fa-check-circle me-1"></i> Stok: {{ $product->stok }}</span>
                                @else
                                    <span class="text-danger"><i class="fas fa-times-circle me-1"></i> Tidak Tersedia</span>
                                @endif
                            </p>
                            ...
                            {{-- AREA TOMBOL AKSI (DISATUKAN) --}}
                            <div class="d-flex align-items-center gap-2 mt-auto">
                                {{-- Tombol Keranjang --}}
                                <div class="flex-grow-1">
                                    @if(!$habis)
                                        <form action="{{ route('tambahKeKeranjang', $product->id) }}" method="POST">
                                            @csrf
                                    ...
                                    @else
                                        <form action="{{ route('favorites.toggle', $product->id) }}" method="POST">
                                            ...
                                            <button type="submit" 
                                                ...
                                                style="color: {{ $hasPurchased ? '#6c757d' : '#dee2e6' }};"
                                                @if(!$hasPurchased && Auth::check()) 
                                                    onclick="alert('Pesan menu ini terlebih dahulu untuk menjadikannya favorit!'); return false;" 
                                                @endif
                                                title="{{ $hasPurchased ? 'Tambah ke favorit' : 'Pesan dulu untuk favorit' }}">
                                                <i class="far fa-heart" style="font-size: 1.5rem;"></i>
                                            </button>
                                        </form>
                                    @endif
                                </div>
                            </div> {{-- End d-flex tombol aksi --}}
